...
1. 请确保您在Windows操作系统下，BVE本体只有适配Windows的版本。
   （虽然在其它操作系统上有可能通过转译等方式运行，但是具体操作方法的复杂程度让这样的方法不属于“新手教程“的范畴。）<br>
   如果你不熟悉使用Windows系统的一些基本操作（复制和移动文件、文件夹，编辑并保存文本文件，压缩包解压等），**强烈建议你先学习一些电脑使用相关的基础知识，再来考虑安装GSR。**<br>
...
<br>（摆式列车目前是一个实验性功能，在线路数据中通过改变轨道超高的方式模拟列车倾斜。仅在博丽神社~南人间之里、命莲寺~守矢神社间有效。）
